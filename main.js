...
import { file_list, get_file } from './rustdrive.mjs'
...
async function updateFiles() {
    ...
    for (const file in files) {
        var li = template.content.querySelector("li").cloneNode(true);
        li.textContent = files[file].filename;
		// set the action based if it is a file or a folder
        if (files[file].is_dir) {
            li.ondblclick = () => openFolder(files[file].filename);
        } else {
            li.ondblclick = () => openFile(files[file].filename);
        }
...
document.getElementById('context-menu-download').onclick = () => {
	...
	items.forEach(function(item) {
		if (item.classList.contains('active')) {
			get_file(new Path( path.as_string() + "/" + item.textContent ).as_string(), true);
		}
	});
...
function openFile(filename) {
	...
	get_file(path.as_string() + "/" + filename, false);
}
